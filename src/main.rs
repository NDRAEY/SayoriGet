...
fn main() {
	...
	match data {
		Ok(values) => {
			...
			if version == None {  // For future
				println!("");
				while (selected < 0) || (selected > arraylen) {
					let mut tempstr = String::new();
					print!("Select entry > ");
					io::stdout().flush().unwrap();
					io::stdin().read_line(&mut tempstr).expect("ERR: Unable to read user input");
			...
			println!("Selected version is: {}", versions[selected]);
			...
			if url == None {
				error(format!("No ISO images found for version: {}", versions[selected]));
				process::exit(1);
			}
